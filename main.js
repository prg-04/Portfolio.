...
lists.map((item) => {
  if (
    item.innerHTML === 'Bootstrap'
		|| item.innerHTML === 'Terminal'
		|| item.innerHTML === 'Codepen'
  ) {
    item.className = popupData[0].classes[7];
  }
});
lists.map((list) => ul.appendChild(list));
...
for (const item of demoLink) {
  item.addEventListener('click', () => {
    window.location.href = popupData[0].btnLinks[0];
  });
}

for (const item of source) {
  item.addEventListener('click', () => {
    window.location.href = popupData[0].btnLinks[1];
  });
}
